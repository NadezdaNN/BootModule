...
from MyThreadNet import *
import keyboard 

class FormSettings(QtWidgets.QWidget, FormSettingsD.Ui_Form):    
    ...
    def __init__(self, parent=None):        
        ...
        self.cx = FormLogo.CX_global
        self.cy = FormLogo.CY_global          
        
        self.ui.widget.move(int(self.cx/2-self.ui.widget.width()/2),int(self.cy/2-self.ui.widget.height()/2))       
        self.ui.editNewPass.setEchoMode(QtWidgets.QLineEdit.Password)        
        self.ui.buttonTakeSett.setText("Сохранить настройки")
        ...
        self.ui.pushButton.setIcon(qApp.style().standardIcon(QStyle.SP_BrowserReload)) 
        self.ui.pushButton_2.setIcon(qApp.style().standardIcon(QStyle.SP_FileIcon))
        ...
        QtWidgets.QWidget.setTabOrder(self.ui.checkBox_2, self.ui.editNetSel)
        QtWidgets.QWidget.setTabOrder(self.ui.editNetSel, self.ui.editPass)
        QtWidgets.QWidget.setTabOrder(self.ui.editPass, self.ui.editNewPass)
        QtWidgets.QWidget.setTabOrder(self.ui.editNewPass, self.ui.buttonTake)
        QtWidgets.QWidget.setTabOrder(self.ui.buttonTake, self.ui.buttonReset) 
        QtWidgets.QWidget.setTabOrder(self.ui.buttonReset, self.ui.editAdrRDP)        
        ...
        QtWidgets.QWidget.setTabOrder(self.ui.editPass2, self.ui.buttonTakeSett)
        ...
        if self.settings.value("checkBox_4.isChecked")=='true': # Старт VPN при загрузке ТК
    ...
    def func(self):
        self.editNewPass = PasswordEdit()       

        
    def slotDelSett(self):
        self.settings.clear()  
        self.settings.setValue("checkBox.isChecked", 'true')            
        self.showSettings()   
        self.settings.setValue("comboBox.text", 'IPsec/L2TP')      
        self.settings.setValue("checkBox_4.isChecked", 'false')             
        self.showSettingsVPN()   
        self.settings.setValue("comboBox_2.text", 'RDP')
        self.showSettingsRDP() 
        os.system("sudo echo '123' > password.py")
    ...
    def eventFilter(self, source, event):    
        ...
        if (event.type() == QEvent.KeyPress and  
                ((event.key()<Qt.Key_0 or event.key()>Qt.Key_9) and event.key()!=Qt.Key_Period and
                event.key()!=Qt.Key_Left and event.key()!=Qt.Key_Right and event.key()!=Qt.Key_Tab and # and event.key()!=Qt.Key_Enter and event.key()!=Qt.Key_Return
                event.key()!=Qt.Key_Escape and event.key()!=Qt.Key_Backspace and event.key()!=Qt.Key_Delete and event.key()!=Qt.Key_Space)):      # and source is self.ui.editDNS     and event.key() != Qt.Key_Backspase    
            return True 
    ...
    def checkBox_clicked(self): # Получение IP автоматически     
        if self.ui.checkBox.isChecked():
            ...
            if (self.settings.value("editIP.text.static") is None):
                self.ui.editIP.setText('...')
            else:
                self.ui.editIP.setText(self.settings.value("editIP.text.static")) 
            if self.settings.value("editNetmask.text.static") is None:
                self.ui.editNetmask.setText('...')
            else:
                self.ui.editNetmask.setText(self.settings.value("editNetmask.text.static")) 
            if self.settings.value("editDHCP.text.static") is None:
                self.ui.editDHCP.setText('...')
            else:
                self.ui.editDHCP.setText(self.settings.value("editDHCP.text.static"))
            if self.settings.value("editDNS.text.static") is None:
                self.ui.editDNS.setText('...')
            else:
                self.ui.editDNS.setText(self.settings.value("editDNS.text.static")) 
        ...
        if self.ui.checkBox_4.isChecked():                       
            ...
            self.ui.comboBox.setStyleSheet("QComboBox"
                                        "{"
                                            ...
                                            "background-color: white;"                                            
                                        "}")                           
            self.ui.comboBox.setCurrentText('IPsec/L2TP')  
    ...
    def comboBox_activated(self): # Выбор VPN
        ...
        if strVPN=="OpenVPN" and self.ui.checkBox_4.isChecked():
            ...
            self.ui.editLog3.setDisabled(False)   
            self.ui.editLog3.setStyleSheet("background-color: white")  
            self.ui.editLog3.setText(self.settings.value("editLog3.text.openvpn"))         
            self.ui.labelNetSel_2.setDisabled(False)            
            self.ui.editPass_2.setDisabled(False)
            self.ui.editPass_2.setStyleSheet("background-color: white")
            self.ui.editPass_2.setText(self.settings.value("editPass_2.text.openvpn")) 
            self.ui.labelPass_2.setDisabled(False)
            self.ui.pushButton_2.setDisabled(False)   
            ...
            self.ui.lineEdit.setDisabled(False) 
            self.ui.lineEdit.setStyleSheet("background-color: white") 
            self.ui.lineEdit.setText(self.settings.value("lineEdit.text.l2tp"))
            self.ui.label_4.setDisabled(False) 
            self.ui.lineEdit_2.setDisabled(False) 
            self.ui.lineEdit_2.setStyleSheet("background-color: white") 
            self.ui.lineEdit_2.setText(self.settings.value("lineEdit_2.text.l2tp"))
            self.ui.label_5.setDisabled(False)                           
            self.ui.editLog3.setDisabled(False)   
            self.ui.editLog3.setStyleSheet("background-color: white")     
            self.ui.editLog3.setText(self.settings.value("editLog3.text.l2tp"))     
            self.ui.labelNetSel_2.setDisabled(False)            
            self.ui.editPass_2.setDisabled(False)
            self.ui.editPass_2.setStyleSheet("background-color: white")
            self.ui.editPass_2.setText(self.settings.value("editPass_2.text.l2tp")) 
            self.ui.labelPass_2.setDisabled(False)
            self.ui.pushButton_2.setDisabled(True)    
            self.ui.pushButton_2.setStyleSheet("background-color: rgb(240, 240, 240)") 
            self.ui.label.setDisabled(True)
    ...
    def comboBox_2_activated(self): # Выбор Подключения Web/RDP
        ...
        self.ui.comboBox_2.setStyleSheet("QComboBox"
                                        ...
                                        "}")  
        if strConnect=="Web":      
            self.ui.lineEdit_3.setText(self.settings.value("lineEdit_3.text")) 
            ...
            self.ui.editAdrRDP.setText(self.settings.value("editAdrRDP.text"))                         
            self.ui.editAdrRDP.setDisabled(False)  
            self.ui.editAdrRDP.setStyleSheet("background-color: white")
            self.ui.labelAdrRDP.setDisabled(False)
            self.ui.editLog2.setText(self.settings.value("editLog2.text"))  
            self.ui.editLog2.setDisabled(False)  
            self.ui.editLog2.setStyleSheet("background-color: white")
            self.ui.labelLog2.setDisabled(False)      
            self.ui.editPass2.setText(self.settings.value("editPass2.text"))  
            self.ui.editPass2.setDisabled(False) 
            self.ui.editPass2.setStyleSheet("background-color: white")
            self.ui.labelPass2.setDisabled(False)
            self.ui.labelSettRDP.setText('') 
            if self.settings.value("checkBox_5.isChecked")=='true':
    ...
    def buttonTake_clicked(self):  #Принять (пароль)       
        if not self.ui.editNewPass.text(): 
    ...
    def buttonTakeSett_clicked(self): #Сохранить настройки        
        self.ui.labelSettEth.setText("") 
        self.ui.labelSettWiFi.setText("") 
        self.ui.labelSettRDP.setText("")        
        ...
        if (self.ui.checkBox_2.isChecked()):            
            ...
            if (str_wifi!="" and len(self.ui.editPass.text())>=8 and len(self.ui.editPass.text())<=64):  
            ...
            if self.ui.checkBox.isChecked(): #динамический IP-адрес     
                if str(self.ui.checkBox.isChecked()).lower() != str(self.settings.value("checkBox.isChecked")):                     
                    self.setCursor(Qt.BusyCursor) 
                    nameUser=os.getlogin()
                    os.system("sudo chmod 755 /etc/dhcpcd.conf")
                    os.system(f"sudo chown {nameUser}:{nameUser} /etc/dhcpcd.conf")
                    file=open('/etc/dhcpcd.conf', 'r+')  
                    ...
                    os.system("sudo chmod 755 /etc/network/interfaces")
                    os.system("sudo chown " + nameUser + ":" + nameUser + " /etc/network/interfaces")
                    with open("/etc/network/interfaces", 'w+') as file:           
                        file.write('auto lo\n')
                        file.write('iface lo inet loopback\n')
                    ...
                    self.settings.setValue("checkBox.isChecked", self.ui.checkBox.isChecked())
                ...
                if flag:
                    if (self.ui.editIP.text()==self.settings.value("editIP.text.static") and 
                            self.ui.editNetmask.text()==self.settings.value("editNetmask.text.static") and
                            self.ui.editDHCP.text()==self.settings.value("editDHCP.text.static") and 
                            self.ui.editDNS.text()==self.settings.value("editDNS.text.static") and
                            str(self.ui.checkBox.isChecked()).lower() == str(self.settings.value("checkBox.isChecked"))):                             
                        pass
                    else:
                        self.setCursor(Qt.BusyCursor) 
                        nameUser=os.getlogin()
                        os.system("sudo chmod 755 /etc/dhcpcd.conf")
                        os.system("sudo chown " + nameUser + ":" + nameUser + " /etc/dhcpcd.conf")
                        file=open('/etc/dhcpcd.conf', 'r+')  
                        ...
                        self.ip_eth0 = self.ui.editIP.text().rstrip('\n') 
                        self.mask_eth0 = self.ui.editNetmask.text().rstrip('\n') 
                        self.dhcp = self.ui.editDHCP.text().rstrip('\n') 
                        self.dns = self.ui.editDNS.text().rstrip('\n')
                        
                        self.settings.setValue("checkBox.isChecked", self.ui.checkBox.isChecked())
                        self.settings.setValue("editIP.text.static", self.ui.editIP.text()) 
                        self.settings.setValue("editNetmask.text.static", self.ui.editNetmask.text()) 
                        self.settings.setValue("editDHCP.text.static", self.ui.editDHCP.text())
                        self.settings.setValue("editDNS.text.static", self.ui.editDNS.text())
        ...
        if self.ui.checkBox_4.isChecked():      
            ...
            if self.ui.comboBox.currentText()=='OpenVPN':      
                if (self.ui.comboBox.currentText()==self.settings.value("comboBox.text") and                        
                        self.ui.editLog3.text()==self.settings.value("editLog3.text.openvpn") and 
                        self.ui.editPass_2.text()==self.settings.value("editPass_2.text.openvpn")):
                    pass
                else:
                    self.settings.setValue("comboBox.text", self.ui.comboBox.currentText())                 
                    self.settings.setValue("editLog3.text.openvpn", self.ui.editLog3.text())
                    self.settings.setValue("editPass_2.text.openvpn", self.ui.editPass_2.text())
                    self.ui.label_2.setStyleSheet("color: black")
                    self.ui.label_2.setText("Настройки сохранены")             
            ...
            if self.ui.comboBox.currentText()=='IPsec/L2TP':
                if (self.ui.comboBox.currentText()==self.settings.value("comboBox.text") and
                        self.ui.lineEdit.text()==self.settings.value("lineEdit.text.l2tp") and 
                        self.ui.lineEdit_2.text()==self.settings.value("lineEdit_2.text.l2tp") and
                        self.ui.editLog3.text()==self.settings.value("editLog3.text.l2tp") and 
                        self.ui.editPass_2.text()==self.settings.value("editPass_2.text.l2tp") and
                        self.ui.editAdrRDP.text()==self.settings.value("editAdrRDP.text")):
                    if str(self.ui.checkBox_4.isChecked()).lower() == str(self.settings.value("checkBox_4.isChecked")):
                        pass
                    else:                
                        self.setCursor(Qt.BusyCursor)        
                        self.signalUpVPN.emit()   
                    ...
                    if (self.ui.lineEdit.text()!='' and self.ui.lineEdit_2.text()!='' and self.ui.editLog3.text()!="" and self.ui.editPass_2.text()!= ""):   
                        ...
                        self.settings.setValue("comboBox.text", self.ui.comboBox.currentText()) 
                        self.settings.setValue("lineEdit.text.l2tp", self.ui.lineEdit.text()) 
                        self.settings.setValue("lineEdit_2.text.l2tp", self.ui.lineEdit_2.text())
                        self.settings.setValue("editLog3.text.l2tp", self.ui.editLog3.text())
                        self.settings.setValue("editPass_2.text.l2tp", self.ui.editPass_2.text())
            ...
            self.settings.setValue("checkBox_4.isChecked", self.ui.checkBox_4.isChecked())            
        else:            
            if str(self.ui.checkBox_4.isChecked()).lower() == str(self.settings.value("checkBox_4.isChecked")):
                pass
            else:
                if self.settings.value("comboBox.text")=='IPsec/L2TP':
                ...
                elif self.settings.value("comboBox.text")=='OpenVPN':
                    pass
        ...
        # Connection RDP/Web
        if self.ui.comboBox_2.currentText()=='Web'  and self.ui.comboBox_2.currentText()!=self.settings.value('comboBox_2.text'):            
            if self.ui.lineEdit_3.text() != '':     
                nameUser=os.getlogin()            
                with open("/home/" + nameUser + "/runrdp", 'w+') as file:    
                    line=self.ui.lineEdit_3.text()                    
                    if ('http://' in line) or ('https://' in line):            
                        file.write(f'sudo -u {nameUser} chromium {self.ui.lineEdit_3.text()} --kiosk') 
                    else:
                        file.write(f'sudo -u {nameUser} chromium https://{self.ui.lineEdit_3.text()} --kiosk')

                self.settings.setValue("comboBox_2.text", self.ui.comboBox_2.currentText())
                self.settings.setValue("lineEdit_3.text", self.ui.lineEdit_3.text())

                self.ui.labelSettRDP.setStyleSheet("color: black")                    
                self.ui.labelSettRDP.setText("Настройки сохранены")
            elif self.ui.lineEdit_3.text() == self.settings.value('lineEdit_3.text'):
                pass
            else:
                self.ui.labelSettRDP.setStyleSheet("color: red")
                self.ui.labelSettRDP.setText("Ввод недопустимых значений")
        
        if self.ui.comboBox_2.currentText()=='RDP' and self.ui.comboBox_2.currentText()!=self.settings.value('comboBox_2.text'):
            flag = True            
            if (self.ui.editAdrRDP.text() != ""):                
                ...
                if (self.ui.editLog2.text().rstrip('\n')=="" and self.ui.editPass2.text().rstrip('\n')==""):
                    if flagUSB:
                        file.write("rdesktop " + self.ui.editAdrRDP.text().rstrip('\n') + " -r disk:USB=/media/ -f -z\n")
                    else:
                        file.write("rdesktop " + self.ui.editAdrRDP.text().rstrip('\n') + " -f -z\n")                    
                if (self.ui.editLog2.text().rstrip('\n')!="" and self.ui.editPass2.text().rstrip('\n')==""):
                    if flagUSB:
                ...
                if (self.ui.editLog2.text().rstrip('\n')!="" and self.ui.editPass2.text().rstrip('\n')!=""):
                    if flagUSB:
                        file.write("rdesktop " + self.ui.editAdrRDP.text().rstrip('\n') + \
                            " -u " + self.ui.editLog2.text().rstrip(' ').rstrip('\n').strip('\n') + \
                            " -p " + self.ui.editPass2.text().rstrip('\n') + " -r disk:USB=/media/ -f -z\n")
                    else:
                        file.write("rdesktop " + self.ui.editAdrRDP.text().rstrip('\n') + \
                            " -u " + self.ui.editLog2.text().rstrip(' ').rstrip('\n').strip('\n') + \
                            " -p " + self.ui.editPass2.text().rstrip('\n') + " -f -z\n")
                ...
                self.settings.setValue("comboBox_2.text", self.ui.comboBox_2.currentText())                     
                self.settings.setValue("editAdrRDP.text", self.ui.editAdrRDP.text()) 
                self.settings.setValue("editLog2.text", self.ui.editLog2.text()) 
                self.settings.setValue("editPass2.text", self.ui.editPass2.text()) 
                self.settings.setValue("checkBox_5.isChecked", self.ui.checkBox_5.isChecked()) 
    ...
    def showSettings(self):        
        if self.settings.value("checkBox.isChecked")=='true':
            self.ui.editIP.setText('...') 
            self.ui.editNetmask.setText('...') 
            self.ui.editDHCP.setText('...')
            self.ui.editDNS.setText('...')      
    ...
    def showSettingsVPN(self):       
        if self.settings.value("checkBox_4.isChecked")=='true':
        ...
        self.ui.comboBox.setCurrentText(self.settings.value("comboBox.text"))          

        if self.ui.comboBox.currentText()=='IPsec/L2TP' and self.settings.value("checkBox_4.isChecked")=='true':                    
            self.ui.lineEdit.setText(self.settings.value("lineEdit.text.l2tp"))
            self.ui.lineEdit_2.setText(self.settings.value("lineEdit_2.text.l2tp"))
            self.ui.editLog3.setText(self.settings.value("editLog3.text.l2tp"))
            self.ui.editPass_2.setText(self.settings.value("editPass_2.text.l2tp"))
        ...
        elif self.ui.comboBox.currentText()=='OpenVPN' and self.settings.value("checkBox_4.isChecked")=='true':    
            self.ui.lineEdit.setText('')
            self.ui.lineEdit_2.setText('')        
            self.ui.editLog3.setText(self.settings.value("editLog3.text.openvpn"))
            self.ui.editPass_2.setText(self.settings.value("editPass_2.text.openvpn"))   
    ...
    def showSettingsRDP(self):    
        self.ui.comboBox_2.setCurrentText(self.settings.value("comboBox_2.text"))
